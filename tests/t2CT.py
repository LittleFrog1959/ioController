...
def main (ui, *args):
    ...
    while True:
        try:
            ...
            logMsg ('Connected without error')
        ...
        logMsg ("now in main program")
        while True:
            logMsg ("client information " + str (type (client)))
            time.sleep (1)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    ui.addstr (0, 0, 'Error')
                    ui.refresh ()
                    logMsg ('main receive loop error')

                if gotStart == False:
                    logMsg ("Haven't got start")
...
def printLine (r, m):
    logMsg ('Processing lines of text')
